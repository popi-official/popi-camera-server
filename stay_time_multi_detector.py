...
class StayTimeDetector2:
    def __init__(self, max_distance=1.2, box_size=50, width=1280, height=720):
        ...
        # 상품 3개 구간 정의
        self.products = {
            "P1": {"cx": width // 4, "cy": height // 2},
            "P2": {"cx": width // 2, "cy": height // 2},
            "P3": {"cx": 3 * width // 4, "cy": height // 2},
        }

        # 상태 변수 초기화
        for zone in self.products.values():
            zone.update({
                "detected": False,
                "start_time": None,
                "distance_sum": 0.0,
    ...
    def get_avg_distance(self, frame, cx, cy):
        distances = []
        for i in range(-self.box_size, self.box_size):
            for j in range(-self.box_size, self.box_size):
                ...
                if 0 <= dx < self.width and 0 <= dy < self.height:
                    depth = frame.get_distance(dx, dy)
    ...
    def run(self):
        try:
            while True:
                ...
                for product, zone in self.products.items():
                    cx, cy = zone["cx"], zone["cy"]
                    avg_distance = self.get_avg_distance(depth_frame, cx, cy)
                    ...
                    if avg_distance is not None and avg_distance <= self.max_distance:
                        ...
                        if zone["detected"]:
                            ...
                            interest = self.classify_interest(avg, duration)
                            if interest:
                                self.publish_event(product, interest, avg, duration)
                            print(f"{product} 영역 손님 이동")

                        # 초기화
                        zone["detected"] = False
                        zone["start_time"] = None
                        zone["distance_sum"] = 0.0
                        zone["frame_count"] = 0
                    ...
                    cv2.putText(color_image, f"{product}: {label}", (cx - 40, cy - self.box_size - 10),
